...
- **支付模块** - 集成了 `sub2apipay`，可直接提供用户自助充值/订阅购买和管理员支付管理
- **推荐系统** - 提供用户邀请返利、奖励记录与后台奖励规则配置
- **模型照妖镜** - 面向普通用户开放的 Claude 中转检测工具，用于检查真实性、结构完整性和注入/覆盖风险
- **模型广场** - 面向普通用户的模型市场，支持跨分组展示可用模型、价格对比、节省分析和多维度筛选
- **分组运行状态** - 实时监控分组模型服务的健康状态，包括可用性追踪、延迟指标和历史事件记录
...
- **内置支付** - 集成 `/pay` 购买/充值流程和管理员支付管理入口
- **推荐系统** - 支持邀请链接、奖励记录和可配置推荐奖励
- **模型照妖镜** - 内置 Claude 中转检测工具，普通用户可直接使用
- **模型广场** - 浏览可用模型，支持价格对比和节省分析
- **分组运行状态** - 实时监控分组模型服务的健康状态和可用性
- **管理后台** - Web 界面进行监控和管理
...
| 项目 | 说明 | 功能 |
|------|------|------|
| [Sub2ApiPay](https://github.com/touwaeriol/sub2apipay) | 仓库内置支付模块 | 本仓库已集成该支付模块，可直接用于一体化部署；支持用户自助充值、自助订阅购买；兼容易支付协议、微信官方支付、支付宝官方支付、Stripe |
