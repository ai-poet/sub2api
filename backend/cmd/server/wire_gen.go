...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	referralRewardRecordRepository := service.ProvideReferralRewardRecordRepository(redeemCodeRepository)
	referralService := service.NewReferralService(referralRepository, referralCache, userRepository, settingRepository, referralRewardRecordRepository, subscriptionService)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, tencentCaptchaService, aliyunCaptchaService, emailQueueService, promoService, subscriptionService, referralService, serviceUserPlatformQuotaRepository)
	...
	availableGroupReader := service.ProvideAvailableGroupReader(apiKeyService)
	groupStatusService := service.NewGroupStatusService(groupStatusRepository, groupRepository, settingService, availableGroupReader)
	...
	groupStatusNotifyService := service.NewGroupStatusNotifyService(settingRepository)
	groupStatusProbeService := service.ProvideGroupStatusProbeService(groupStatusRepository, groupRepository, schedulerSnapshotService, accountTestService, gatewayService, openAIGatewayService, groupStatusNotifyService)
	groupHandler := handler.ProvideAdminGroupHandler(adminService, dashboardService, groupCapacityService, groupStatusService, groupStatusProbeService, configConfig)
	...
	invoiceStorageSettingService := service.NewInvoiceStorageSettingService(settingRepository, secretEncryptor, backupService)
	ticketAttachmentStorageSettingService := service.NewTicketAttachmentStorageSettingService(settingRepository, secretEncryptor, backupService)
	backupHandler := admin.NewBackupHandler(backupService, userService, imageStorageSettingService, invoiceStorageSettingService, ticketAttachmentStorageSettingService)
	...
	settingHandler := handler.ProvideAdminSettingHandler(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, userAttributeService, notificationEmailService, totpService, userService, groupStatusNotifyService)
	...
	modelCatalogService := service.NewModelCatalogService(apiKeyService, gatewayService, billingService, modelPricingResolver)
	modelCatalogHandler := handler.NewModelCatalogHandler(modelCatalogService)
	publicPricingHandler := handler.NewPublicPricingHandler(modelCatalogService, settingService)
	...
	payAttachmentStoreFactory := repository.NewPayAttachmentStoreFactory()
	payAttachmentService := service.NewPayAttachmentService(invoiceStorageSettingService, payAttachmentStoreFactory)
	payInvoiceNotifyService := service.NewPayInvoiceNotifyService(notificationEmailService, userService)
	payBridgeHandler := handler.NewPayBridgeHandler(payAttachmentService, payInvoiceNotifyService)
	handlerTicketHandler := handler.NewTicketHandler(ticketService)
	handlerTicketAttachmentHandler := handler.NewTicketAttachmentHandler(ticketAttachmentService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerReferralHandler, modelCatalogHandler, publicPricingHandler, groupStatusHandler, passkeyHandler, availableChannelHandler, asyncImageHandler, batchImageHandler, payBridgeHandler, handlerTicketHandler, handlerTicketAttachmentHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.ProvideAdminAuthMiddleware(authService, userService, settingService, auditLogService, adminApprovalService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, userService, opsService, settingService, referralService, compositeRouteResolver, redisClient, adminApprovalService)
