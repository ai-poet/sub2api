...
// 更新日志改由 GitHub Releases 提供（ClientChangelogService），公开设置和 HTML 注入
// 里都不再带条目；后台填写的仓库也不对外暴露。
func TestSettingService_GetPublicSettingsForInjection_OmitsClientChangelog(t *testing.T) {
	repo := &settingPublicRepoStub{
		values: map[string]string{
	...
	encoded, err := json.Marshal(payload)
	require.NoError(t, err)
	require.NotContains(t, string(encoded), "client_changelog")
	require.NotContains(t, string(encoded), "agent-client")
}
