...
type CreateUserRequest struct {
	...
	Role                 string   `json:"role" binding:"omitempty,oneof=admin user operator"`
...
func (h *UserHandler) Create(c *gin.Context) {
	...
	// 创建管理员 / 运维管理员账号属权限敏感操作：需最近完成 step-up 2FA 验证。
	if service.IsPrivilegedRole(req.Role) {
...
func (h *UserHandler) Update(c *gin.Context) {
	...
	// 防锁死保护：管理员不能把自己降级为普通用户或运维管理员(单管理员场景下会失去后台访问权)。
	...
	// 把用户改为管理员 / 运维管理员属权限敏感操作：需最近完成 step-up 2FA 验证。
	// 目标已是该角色时（前端编辑表单总是携带 role）不触发，避免日常编辑被打断。
	if service.IsPrivilegedRole(req.Role) {
