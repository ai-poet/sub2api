    ...
    "approval": {
      ...
      "describe": {
        ...
        "summary": {
          ...
          "userUpdate": "修改用户 {target} 的资料",
          "balanceAdd": "给 {target} 增加余额 {amount}",
          "balanceSubtract": "从 {target} 扣减余额 {amount}",
          "balanceSet": "把 {target} 的余额设为 {amount}",
          "replaceGroup": "把 {target} 的专属分组从「{from}」换成「{to}」",
          "batchConcurrencySet": "把 {scope} 的并发数设为 {value}",
          "batchConcurrencyAdd": "给 {scope} 的并发数增加 {value}",
          "batchLimits": "批量调整 {scope} 的限额",
          "platformQuotas": "修改 {target} 的平台额度",
          "platformQuotaReset": "重置 {target} 在 {platform} 的{window}额度窗口",
          "attributes": "修改 {target} 的用户属性",
          "authIdentity": "为 {target} 绑定 {provider} 登录身份",
          "apiKeyBind": "把 API Key {target} 绑定到分组「{group}」",
          "apiKeyUnbind": "解绑 API Key {target} 的分组",
          "apiKeyUpdate": "调整 API Key {target}",
          "subscriptionAssign": "分配订阅：{target}",
          "subscriptionBulkAssign": "给 {count} 个用户分配分组「{group}」的订阅",
          "subscriptionExtend": "把订阅 {target} 延长 {days} 天",
          "subscriptionShorten": "把订阅 {target} 缩短 {days} 天",
          "subscriptionResetQuota": "重置订阅 {target} 的{windows}额度",
