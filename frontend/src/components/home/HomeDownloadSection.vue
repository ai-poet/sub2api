...
const platformRank: Record<DownloadPlatform, number> = {
  windows: 0,
  macos: 1
}
...
const preferredPlatform = computed<DownloadPlatform>(() => {
...
const configuredOptions = computed<DownloadOption[]>(() => {
  const windowsUrl = props.windowsUrl?.trim() || ''
  const macosUrl = props.macosUrl?.trim() || ''
  ...
  if (windowsUrl) {
    options.push({
      id: 'windows',
      name: 'Windows',
      sub: t('home.download.platforms.windows.sub'),
      url: windowsUrl,
      iconPath: platformIcons.windows
    })
  }
  ...
  if (macosUrl) {
    options.push({
      id: 'macos',
      name: 'macOS',
      sub: t('home.download.platforms.mac.sub'),
      url: macosUrl,
      iconPath: platformIcons.macos
    })
  }
...
const downloadOptions = computed(() =>
  [...configuredOptions.value].sort((a, b) => {
    if (a.id === preferredPlatform.value && b.id !== preferredPlatform.value) return -1
    if (b.id === preferredPlatform.value && a.id !== preferredPlatform.value) return 1
