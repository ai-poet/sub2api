<template>
  <section class="mx-auto max-w-[1380px]">
    <div class="max-w-[980px]">
      ...
      <h2 class="mt-4 max-w-[16ch] text-4xl font-semibold leading-[1.1] tracking-[-0.04em] text-gray-900 dark:text-white md:text-5xl [text-wrap:balance]">
    ...
    <div class="mt-14 grid gap-10 border-t border-black/10 pt-8 dark:border-white/10 lg:grid-cols-3 lg:gap-8">
      <article
        ...
        :key="item.title"
        class="border-b border-black/10 pb-8 dark:border-white/10 lg:border-b-0 lg:pb-0"
      >
        <div class="flex items-center justify-between gap-4">
          <p class="text-[11px] uppercase tracking-[0.22em] text-gray-500 dark:text-white/38">
            {{ item.eyebrow }}
          </p>
          <div class="flex h-11 w-11 items-center justify-center rounded-full border border-black/10 bg-white/70 text-gray-900 dark:border-white/10 dark:bg-white/5 dark:text-white">
            <Icon :name="item.icon" size="md" />
          </div>
        </div>

        <h3 class="mt-8 text-2xl font-semibold leading-tight tracking-[-0.04em] text-gray-900 dark:text-white">
          {{ item.title }}
        </h3>
        <p class="mt-4 text-base leading-7 text-gray-600 dark:text-white/65">
          {{ item.description }}
        </p>
      </article>
    </div>
