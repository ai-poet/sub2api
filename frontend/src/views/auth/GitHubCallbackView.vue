    ...
    <div class="space-y-6">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.github.callbackTitle') }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ isProcessing ? t('auth.github.callbackProcessing') : t('auth.github.callbackHint') }}
        </p>
      </div>
      ...
      <transition name="fade">
        <div v-if="needsInvitation" class="space-y-4">
          <p class="text-sm text-gray-700 dark:text-gray-300">
            {{ t('auth.github.invitationRequired') }}
          </p>
          <div>
            <input
          ...
          >
            {{ isSubmitting ? t('auth.github.completing') : t('auth.github.completeRegistration') }}
          </button>
        </div>
      </transition>
...
onMounted(async () => {
  ...
  if (!token) {
    errorMessage.value = t('auth.github.callbackMissingToken')
    appStore.showError(errorMessage.value)
    isProcessing.value = false
    return
  }
