...
describe('admin SettingsView client downloads', () => {
  beforeEach(() => {
    ...
    getSettings.mockResolvedValue(createSettings())
  ...
  it('loads client download URLs and submits updated values', async () => {
    ...
    await windowsInput.setValue('https://cdn.example.com/Sub2API-Setup.exe')
    await macosInput.setValue('https://cdn.example.com/Sub2API.dmg')
    await wrapper.find('form').trigger('submit')
    await flushPromises()

    expect(updateSettings).toHaveBeenCalledWith(
      expect.objectContaining({
        client_download_windows_url: 'https://cdn.example.com/Sub2API-Setup.exe',
        client_download_macos_url: 'https://cdn.example.com/Sub2API.dmg',
      }),
    )
  ...
  it('clears invalid client download URLs before submitting', async () => {
    const wrapper = mountSettingsView()
    await flushPromises()

    await wrapper.find<HTMLInputElement>('[data-test="client-download-windows-url"]').setValue('ftp://bad.example.com/app.exe')
    await wrapper.find<HTMLInputElement>('[data-test="client-download-macos-url"]').setValue('not a url')
    await wrapper.find('form').trigger('submit')
    await flushPromises()
