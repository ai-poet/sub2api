...
import type { AdminGroup, ClientChangelogEntry } from '@/types'
...
// Client changelog management
const changelogPreviewIndex = ref<string | null>(null)

function addChangelogEntry() {
  form.client_changelog_entries.unshift({
    version: '',
...
function removeChangelogEntry(index: number) {
  ...
  form.client_changelog_entries.splice(index, 1)
}

function moveChangelogEntry(index: number, direction: -1 | 1) {
  const targetIndex = index + direction
  if (targetIndex < 0 || targetIndex >= form.client_changelog_entries.length) return
  const entries = form.client_changelog_entries
...
function addChangelogItem(entryIndex: number) {
  form.client_changelog_entries[entryIndex].items.push('')
  nextTick(() => {
...
function removeChangelogItem(entryIndex: number, itemIndex: number) {
  form.client_changelog_entries[entryIndex].items.splice(itemIndex, 1)
}
...
function normalizeChangelogEntries(
  entries?: ClientChangelogEntry[] | null
): ClientChangelogEntry[] {
  if (!Array.isArray(entries)) return []
  return entries.map((entry) => ({
    version: (entry.version || '').trim(),
    published_at: (entry.published_at || '').trim(),
    title: (entry.title || '').trim(),
    items: Array.isArray(entry.items)
      ? entry.items.map((item) => (item || '').trim()).filter(Boolean)
      : [],
...
async function loadSettings() {
  ...
  try {
    ...
    form.client_changelog_entries = normalizeChangelogEntries(
      settings.client_changelog_entries
    )
    registrationEmailSuffixWhitelistDraft.value = ''
    form.smtp_password = ''
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      client_changelog_entries: normalizeChangelogEntries(form.client_changelog_entries)
        .filter((entry) => entry.version || entry.title || entry.items.length > 0),
    ...
    form.client_changelog_entries = normalizeChangelogEntries(
      updated.client_changelog_entries
    )
    registrationEmailSuffixWhitelistDraft.value = ''
    form.smtp_password = ''
    smtpPasswordManuallyEdited.value = false
