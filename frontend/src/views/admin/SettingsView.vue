    ...
    <div class="mx-auto max-w-6xl space-y-6">
      ...
      <form v-else @submit.prevent="saveSettings" class="space-y-6" novalidate>
        ...
        <div class="card">
          <div class="border-b border-gray-100 px-6 py-4 dark:border-dark-700">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white">
              {{ t('admin.settings.features.publicPricing.title') }}
            </h2>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {{ t('admin.settings.features.publicPricing.description') }}
            </p>
          </div>
          <div class="space-y-5 p-6">
            <div class="flex items-center justify-between">
              <div>
                <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
                  {{ t('admin.settings.features.publicPricing.enabled') }}
                </label>
                <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                  {{ t('admin.settings.features.publicPricing.enabledHint') }}
                </p>
              </div>
              <Toggle v-model="form.public_pricing_enabled" />
            </div>
          </div>
        </div>
...
async function saveSettings() {
  ...
  try {
    ...
    const payload: UpdateSettingsRequest = {
      ...
      // Public pricing catalog on the landing page
      public_pricing_enabled: form.public_pricing_enabled,
...
.settings-tab::before {
  ...
  content: "";
  background: linear-gradient(135deg, rgb(248 247 245 / 0.95), rgb(242 240 236 / 0.8));
}
...
.dark .settings-tabs-shell {
  border-color: rgb(48 44 40 / 0.65);
...
.dark .settings-tab::before {
  background: linear-gradient(135deg, rgb(35 38 41 / 0.9), rgb(48 44 40 / 0.62));
}
