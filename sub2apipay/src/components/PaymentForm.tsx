...
export default function PaymentForm({
  ...
  const renderPaymentIcon = (type: string) => {
    ...
    if (iconType === 'usdt') {
      return (
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#26A17B] text-xs font-bold text-white">
          USDT
        </span>
      );
    }
    if (iconType === 'usdc') {
      return (
        <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#2775CA] text-xs font-bold text-white">
          USDC
        </span>
      );
    }
