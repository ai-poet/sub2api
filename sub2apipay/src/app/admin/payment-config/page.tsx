...
function getTexts(locale: Locale) {
  return locale === 'en'
    ? {
        ...
        basicConfigHint: 'Recharge, order lifecycle, and provider settings stored in the database.',
        ...
        overrideEnvConfig: 'Runtime Config',
        overrideEnvHint: 'These values are stored in the database and take effect without manual payment env setup.',
        enabledProviders: 'Enabled Provider Types',
        minRechargeAmount: 'Min Credited Balance (USD)',
        maxRechargeAmount: 'Max Credited Balance (USD)',
        dailyRechargeLimit: 'Daily Credited Balance Limit (USD, 0=unlimited)',
    ...
    : {
        ...
        basicConfig: '基础配置',
        basicConfigHint: '充值、订单生命周期和服务商参数统一保存在数据库中。',
        ...
        overrideEnvConfig: '运行配置',
        overrideEnvHint: '以下支付参数统一保存到数据库，首次上线后在后台配置即可，不再依赖额外支付环境变量。',
        enabledProviders: '启用的服务商类型',
        minRechargeAmount: '最小到账金额（USD）',
        maxRechargeAmount: '最大到账金额（USD）',
...
const PAYMENT_TYPE_LABELS: Record<string, { zh: string; en: string }> = {
  ...
  alipay_direct: { zh: '支付宝直连', en: 'Alipay Direct' },
  wxpay: { zh: '微信支付', en: 'WeChat Pay' },
  wxpay_direct: { zh: '微信直连', en: 'WeChat Direct' },
  'usdt.plasma': { zh: 'USDT (Plasma)', en: 'USDT (Plasma)' },
...
const PROVIDER_CONFIG_FIELDS: Record<string, ConfigFieldDef[]> = {
  easypay: [
    ...
    { key: 'cid', label: { en: 'Default Channel ID', zh: '默认渠道 ID' }, sensitive: false, optional: true },
...
function PaymentConfigContent() {
  ...
  const [, setRcOverrideSaved] = useState(false);
    ...
    () =>
      Array.from(
        new Set([
          ...
          ...instances.map((instance) => instance.providerKey).filter((k) => k in PROVIDER_LABELS),
        ]),
      ),
    [instances, rcEnabledProviders],
  );
      ...
      const res = await fetch(buildAppApiPath('/api/admin/config'), {
        ...
        body: JSON.stringify({
          configs: [
            ...
            { key: 'RECHARGE_MIN_AMOUNT', value: rcMinAmount, group: 'payment', label: '最小到账金额（USD）' },
      ...
      <div className={cardCls}>
        ...
        <div className={subCardCls}>
          <div className="flex items-center gap-3 mb-2">
            <Toggle value={rcOverrideEnv} onChange={handleOverrideEnvToggle} disabled />
