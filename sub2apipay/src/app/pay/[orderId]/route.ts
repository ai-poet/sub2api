...
function buildAppUrl(appUrl: string, pathname = '/pay'): string {
  return new URL(pathname, appUrl).toString();
}

function buildResultUrl(appUrl: string, orderId: string): string {
  return buildOrderResultUrl(appUrl, orderId);
}
...
function renderErrorPage(appUrl: string, title: string, message: string, orderId?: string, status = 400): NextResponse {
  const html = renderHtml(
    ...
    `<main class="card">
      ...
      <a class="button secondary" href="${escapeHtml(buildAppUrl(appUrl))}">返回支付首页</a>
    </main>`,
  );
...
function renderStatusPage(appUrl: string, order: ShortLinkOrderStatus): NextResponse {
  ...
  const html = renderHtml(
    ...
    `<main class="card">
      ...
      <a class="button secondary" href="${escapeHtml(buildResultUrl(appUrl, order.id))}">查看订单结果</a>
    </main>`,
  );
...
function renderRedirectPage(appUrl: string, orderId: string, payUrl: string): NextResponse {
  const html = renderHtml(
    '正在跳转支付宝',
    `<main class="card">
      ...
      <a class="text-link" href="${escapeHtml(buildResultUrl(appUrl, orderId))}">已支付？查看订单结果</a>
      <script>
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ orderId: string }> }) {
  ...
  if (!order) {
    return renderErrorPage(appUrl, '订单不存在', '未找到对应订单，请确认二维码是否正确', orderId, 404);
  }

  if (order.paymentType !== 'alipay_direct') {
    return renderErrorPage(appUrl, '支付方式不匹配', '该订单不是支付宝直连订单，无法通过当前链接支付', orderId, 400);
  }

  if (order.status !== ORDER_STATUS.PENDING) {
    return renderStatusPage(appUrl, order);
  }

  if (order.expiresAt.getTime() <= Date.now()) {
    return renderStatusPage(appUrl, {
      id: order.id,
  ...
  if (!Number.isFinite(payAmount) || payAmount <= 0) {
    return renderErrorPage(appUrl, '订单金额异常', '订单金额无效，请返回原页面重新发起支付', order.id, 500);
  }

  // 构建支付商品名称（与 order/service.ts 逻辑保持一致）
  let subject: string;
  if (order.orderType === 'subscription' && order.plan) {
    subject = order.plan.productName || buildDefaultSubscriptionSubject(order.plan.name);
  } else {
    ...
    if (prefix || suffix) {
      ...
      subject = buildDefaultRechargeSubject(payAmount.toFixed(2));
    }
  }

  const instanceConfig = order.providerInstanceId ? await getInstanceConfig(order.providerInstanceId) : undefined;
  const payUrl = buildAlipayPaymentUrl({
    orderId: order.id,
    amount: payAmount,
    subject,
    notifyUrl: `${appUrl}/api/alipay/notify${order.providerInstanceId ? `?inst=${order.providerInstanceId}` : ''}`,
    returnUrl: isAlipayAppRequest(request) ? null : buildResultUrl(appUrl, order.id),
    isMobile: isMobileRequest(request),
    instanceConfig: instanceConfig ?? undefined,
  });

  return renderRedirectPage(appUrl, order.id, payUrl);
}
