...
export async function createOrder(input: CreateOrderInput): Promise<CreateOrderResult> {
  ...
  try {
    ...
    if (instanceResult) {
      ...
      if (instanceProvider) {
        actualProvider = instanceProvider;
      }
    ...
    const orderResultUrl = buildOrderResultUrl(appUrl, order.id, input.userId);
    ...
    if (actualProvider.providerKey === 'easypay') {
      ...
      notifyUrl = `${appUrl}/api/alipay/notify${selectedInstanceId ? `?inst=${selectedInstanceId}` : ''}`;
    } else if (actualProvider.providerKey === 'wxpay') {
      notifyUrl = `${appUrl}/api/wxpay/notify${selectedInstanceId ? `?inst=${selectedInstanceId}` : ''}`;
    }
  ...
  if (paymentTradeNo && paymentType) {
    try {
      ...
      if (providerInstanceId) {
        provider = await getProviderForInstance(paymentType, providerInstanceId);
      }
      if (!provider) {
        await ensureDBProviders();
