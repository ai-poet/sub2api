...
const envSchema = z.object({
  ...
  SUB2API_INTERNAL_BASE_URL: optionalTrimmedString,
  SUB2API_BASE_URL: optionalTrimmedString,
  SUB2API_ADMIN_API_KEY: optionalTrimmedString,
  ...
  MAX_DAILY_AMOUNT_STRIPE: z
    ...
    .transform((v) => (v !== undefined ? Number(v) : undefined))
    .pipe(z.number().min(0).optional()),
  ADMIN_TOKEN: optionalTrimmedString,

  NEXT_PUBLIC_APP_URL: optionalTrimmedString,
...
export function getSub2ApiInternalBaseUrl(): string {
  const env = getEnv();
