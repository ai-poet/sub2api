...
function registerFromList(providers: string[], env: Env, strict: boolean): void {
  ...
  if (providers.includes('alipay') && !registeredKeys.has('alipay')) {
    if (!env.ALIPAY_APP_ID || !env.ALIPAY_PRIVATE_KEY || !env.ALIPAY_PUBLIC_KEY) {
      if (strict)
        throw new Error(
          'PAYMENT_PROVIDERS includes alipay but required env vars are missing: ALIPAY_APP_ID, ALIPAY_PRIVATE_KEY, ALIPAY_PUBLIC_KEY',
        );
  ...
  if (providers.includes('wxpay') && !registeredKeys.has('wxpay')) {
    if (
      ...
      if (strict)
        throw new Error(
          'PAYMENT_PROVIDERS includes wxpay but required env vars are missing: WXPAY_APP_ID, WXPAY_MCH_ID, WXPAY_PRIVATE_KEY, WXPAY_API_V3_KEY, WXPAY_PUBLIC_KEY, WXPAY_PUBLIC_KEY_ID, WXPAY_CERT_SERIAL',
        );
...
export async function ensureDBProviders(): Promise<void> {
  ...
  const dbInstances = await prisma.paymentProviderInstance.findMany({
    where: { enabled: true },
    select: { id: true, providerKey: true, config: true },
  });
  ...
  const providerKeys = [
    ...new Set([
      ...env.PAYMENT_PROVIDERS,
      ...dbProviders,
      ...dbInstances.map((instance) => instance.providerKey),
    ]),
  ];

  registerFromList(providerKeys, env, false);

  for (const key of providerKeys) {
    if (registeredKeys.has(key)) continue;
    const instance = dbInstances.find((item) => item.providerKey === key);
    ...
    switch (key) {
      ...
      case 'alipay':
        if (config.appId && config.privateKey && config.publicKey) {
          paymentRegistry.register(new AlipayProvider(instance.id, config));
          registeredKeys.add(key);
        }
        break;
      case 'wxpay':
        if (
          ...
          paymentRegistry.register(new WxpayProvider(instance.id, config));
          registeredKeys.add(key);
        }
        break;
    }
