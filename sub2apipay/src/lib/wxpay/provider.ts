...
export class WxpayProvider implements PaymentProvider {
  ...
  private readonly instanceConfig?: Record<string, string>;

  constructor(instanceId?: string, instanceConfig?: Record<string, string>) {
    this.instanceId = instanceId;
    this.instanceConfig = instanceConfig;
    this.name = instanceId ? `wxpay-direct:${instanceId}` : 'wxpay-direct';
  }
  ...
  async verifyNotification(
    ...
    const valid = await verifyNotifySign({ timestamp, nonce, body, serial, signature, instanceConfig: this.instanceConfig });
    if (!valid) {
