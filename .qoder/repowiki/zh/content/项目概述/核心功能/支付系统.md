...
- [locale.ts](file://sub2apipay/src/lib/locale.ts)
- [layout.tsx](file://sub2apipay/src/app/layout.tsx)
- [page.tsx](file://sub2apipay/src/app/page.tsx)
- [pay-page.tsx](file://sub2apipay/src/app/pay/page.tsx)
- [pay-result-page.tsx](file://sub2apipay/src/app/pay/result/page.tsx)
- [pay-stripe-popup-page.tsx](file://sub2apipay/src/app/pay/stripe-popup/page.tsx)
</cite>

## 更新摘要
**所做更改**
- 更新了用户路由本地化逻辑，现仅基于 lang 查询参数进行语言解析
- 简化了 API URL 构建逻辑，移除了 locale 参数的处理
- 更新了支付页面组件，增强了语言参数传递和处理机制
- 新增了支付页面布局中的语言检测和主题支持
...
**更新** 本次更新反映了应用变更：API URL 构建逻辑简化，移除了 locale 参数；用户路由本地化逻辑增强，仅基于 lang 查询参数；支付页面组件更新以包含语言参数。
...
- **新增** 语言本地化系统：基于 lang 查询参数的路由本地化，支持简体中文和英文的智能切换。

**章节来源**
- [payment-flow.test.ts:87-93](file://sub2apipay/src/__tests__/payment-flow.test.ts#L87-L93)
- [billing_cache.go:137-143](file://backend/internal/repository/billing_cache.go#L137-L143)
- [billing_service.go:117-136](file://backend/internal/service/billing_service.go#L117-L136)
- [locale.ts:1-29](file://sub2apipay/src/lib/locale.ts#L1-L29)
...
- **更新** 语言本地化系统通过 lang 查询参数实现精准的语言切换，支持用户自定义语言偏好。
...
U->>FE : "访问支付页面(lang=查询参数)"
FE->>FE : "解析lang参数并设置语言"
FE->>GW : "调用网关下单/生成跳转URL/二维码"
GW-->>FE : "返回支付链接/二维码"
FE-->>U : "展示支付页面/二维码(对应语言)"
...
- [payment-alipay.md:64-83](file://sub2apipay/docs/payment-alipay.md#L64-L83)
- [payment-wxpay.md:81-104](file://sub2apipay/docs/payment-wxpay.md#L81-L104)
- [layout.tsx:17-21](file://sub2apipay/src/app/layout.tsx#L17-L21)
- [page.tsx:8-10](file://sub2apipay/src/app/page.tsx#L8-L10)
...
- **新增** 语言本地化：通过 lang 查询参数实现精准的语言切换，支持简体中文和英文的智能识别与应用。
...
- [locale.ts:3-14](file://sub2apipay/src/lib/locale.ts#L3-L14)
...
### **新增** 语言本地化系统
- **路由本地化**：首页根据 lang 查询参数重定向到对应的支付页面，支持 en 和 zh 两种语言版本。
- **布局语言检测**：根布局从请求头中提取 lang 参数，设置相应的 HTML lang 属性和语言环境。
- **组件语言处理**：支付页面、结果页面、Stripe 弹窗等组件都支持从 URL 参数中解析语言设置。
- **语言参数应用**：通过 applyLocaleToSearchParams 函数将语言参数应用到所有 API 请求中。

**章节来源**
- [page.tsx:8-10](file://sub2apipay/src/app/page.tsx#L8-L10)
- [layout.tsx:17-21](file://sub2apipay/src/app/layout.tsx#L17-L21)
- [pay-page.tsx:1162-1181](file://sub2apipay/src/app/pay/page.tsx#L1162-L1181)
- [pay-result-page.tsx:196-203](file://sub2apipay/src/app/pay/result/page.tsx#L196-L203)
- [pay-stripe-popup-page.tsx:19-39](file://sub2apipay/src/app/pay/stripe-popup/page.tsx#L19-L39)
- [locale.ts:24-29](file://sub2apipay/src/lib/locale.ts#L24-L29)

## 依赖分析
- 前端 Provider 依赖各自网关 SDK 与通用签名/编码工具。
- 回调端点依赖后端计费服务进行状态更新与额度扣减。
- 计费服务依赖 Redis 缓存仓库，使用 Lua 脚本保证原子性。
- **新增** 语言本地化系统依赖 URLSearchParams 和请求头信息进行语言参数解析。
...
Locale["语言本地化系统"] --> Layout["根布局"]
Locale --> PayPage["支付页面"]
Locale --> ResultPage["结果页面"]
Locale --> StripePopup["Stripe弹窗"]
```
...
- **新增** 语言参数缓存：通过简化的 URL 构建逻辑，减少不必要的参数处理开销。
...
- **新增** 语言显示异常
  - 检查 URL 中的 lang 参数是否正确传递；确认 resolveLocale 函数能够正确解析语言代码；验证 HTML lang 属性设置。
...
## 结论
Sub2API 支付系统通过前端直连网关与后端回调处理相结合的方式，实现了支付宝与微信支付的直连对接，并提供了完善的订单生命周期管理、安全验签与解密、自动跳转与降级策略、以及基于 Redis 的高性能计费与缓存能力。**更新** 最新版本增强了语言本地化功能，通过简化的 API URL 构建逻辑和基于 lang 查询参数的路由本地化，提供了更加精准和高效的国际化支持。建议在生产环境中强化回调幂等与对账机制，完善监控与告警，持续优化前端用户体验与支付成功率。
...
- **新增** 语言本地化配置
  - 支付页面路由：/pay?lang=zh 或 /pay?lang=en
  - 语言参数解析：resolveLocale 函数支持 zh、en 语言代码识别
  - HTML 属性设置：根据语言自动设置 lang="zh-CN" 或 lang="en"
