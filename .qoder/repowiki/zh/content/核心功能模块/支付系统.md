...
- [sub2apipay/src/lib/pay-utils.ts](file://sub2apipay/src/lib/pay-utils.ts)
- [sub2apipay/src/lib/payment/visibility.ts](file://sub2apipay/src/lib/payment/visibility.ts)
- [sub2apipay/src/app/api/user/route.ts](file://sub2apipay/src/app/api/user/route.ts)
...
- [backend/internal/repository/groupstatusrecord.go](file://backend/internal/repository/groupstatusrecord.go)
- [backend/internal/repository/groupstatusstate.go](file://backend/internal/repository/groupstatusstate.go)
- [backend/internal/repository/idempotencyrecord.go](file://backend/internal/repository/idempotencyrecord......)
</cite>

## 更新摘要
**变更内容**
- 支付可见性逻辑重构：移除了本地化依赖，采用更简洁的可见性判断机制
- 支付页面组件更新：新增语言参数支持，增强国际化显示能力
- 支付表单组件：支持显式语言参数，改善用户体验
- 支付二维码组件：增强语言参数处理，支持多语言界面显示
...
**更新** 本次更新重点关注支付可见性逻辑重构和支付页面组件的语言参数支持，提升了系统的国际化能力和用户体验。
...
- **支付可见性管理**：重构后的可见性逻辑，支持按实例配置的支付方式过滤。

**更新** 新增支付可见性管理组件，移除了原有的本地化依赖，采用更直接的可见性判断机制。
...
### 支付可见性管理
**更新** 支付可见性逻辑已重构，移除了本地化依赖，采用更简洁的可见性判断机制。

- 可见性判断：基于支付实例配置动态过滤可用的支付方式。
- 实例支持：检查支付提供商实例的启用状态和支持的支付类型。
- 配置优先级：支持系统配置的支付方式白名单过滤。
- 返回结果：返回当前环境下实际可用的支付方式列表。

```mermaid
flowchart TD
Start(["开始"]) --> LoadDB["加载支付提供商实例"]
LoadDB --> GetSupported["获取支持的支付类型"]
GetSupported --> GetConfigured["获取配置的支付类型"]
GetConfigured --> FilterConfig["按配置过滤类型"]
FilterConfig --> CheckInstances["检查实例支持情况"]
CheckInstances --> Visible["返回可见的支付类型"]
Visible --> End(["结束"])
```
...
### 前端交互与支付流程
**更新** 支付页面组件已更新以支持语言参数，增强了国际化显示能力。

- 支付表单：支持显式语言参数，选择支付方式、输入金额、选择币种，提交后获取支付参数。
- 支付二维码：支持多语言界面显示，扫码/条码支付场景生成二维码，引导用户完成支付。
- 支付结果页：根据订单状态跳转成功/失败页，支持手动刷新状态。
- 订单列表：分页查询、筛选、导出订单，支持多语言显示。

```mermaid
flowchart TD
User["用户"] --> Form["支付表单<br/>支持语言参数"]
Form --> Submit["提交支付请求"]
Submit --> QR["支付二维码<br/>多语言显示"]
QR --> Pay["用户完成支付"]
Pay --> Result["支付结果页<br/>状态跟踪"]
Result --> Success["成功/失败页面"]
```
...
- **支付可见性优化**：重构后的可见性逻辑减少了不必要的本地化处理开销。
...
- **支付可见性问题**：检查支付提供商实例配置、支持类型映射和系统配置。
...
## 结论
该支付系统以"前端统一入口 + 后端网关适配 + 计费仓储"为核心，实现了多支付渠道的统一接入与闭环管理。通过严格的验签、幂等与对账机制，保障了交易安全与账实一致；通过用量与账单的自动化处理，支撑了订阅与按量计费的灵活组合。

**更新** 最新重构显著提升了系统的国际化能力和用户体验，支付可见性逻辑更加简洁高效，支付页面组件的语言参数支持使得多语言环境下的支付体验更加流畅。

建议持续完善风控策略、监控体系与灰度发布流程，进一步提升系统的稳定性与可维护性。
